...
public class MeshExtrusion
{
    ...
    public static void Extrude(GameObject gameObject, float extrusion, bool invertFaces)
    {
        ...
        if (mf != null)
        {
            ...
            Edge[] precomputedEdges = BuildManifoldEdges(srcMesh);
            ...
            finalSections[0] = worldToLocal * Matrix4x4.TRS(currentPosition, direction, gameObject.transform.lossyScale);
            finalSections[1] = worldToLocal * Matrix4x4.TRS(targetPosition, direction, gameObject.transform.lossyScale);

            ExtrudeMesh(srcMesh, gameObject.GetComponent<MeshFilter>().sharedMesh, finalSections, precomputedEdges, invertFaces);
        }
    }
    ...
    /// <param name="invertFaces"></param>
    public static void ExtrudeMesh(Mesh srcMesh, Mesh extrudedMesh, float extrusion, Transform currentTransform, bool invertFaces = true)
    {
        Edge[] edges = BuildManifoldEdges(srcMesh);

        Vector3 extrusionNormal = currentTransform.up;

        // first section - on the ground
        Vector3 currentPosition = currentTransform.position;

        // second section - roof
        Vector3 targetPosition = currentPosition + new Vector3(0f, extrusion, 0f);


        Matrix4x4 worldToLocal = currentTransform.worldToLocalMatrix;
        Quaternion direction = Quaternion.LookRotation(extrusionNormal/*, Vector3.up*/);

        Matrix4x4[] finalSections = new Matrix4x4[2];
        finalSections[0] = worldToLocal * Matrix4x4.TRS(currentPosition, direction, Vector3.one);
        finalSections[1] = worldToLocal * Matrix4x4.TRS(targetPosition, direction, Vector3.one);

        ExtrudeMesh(srcMesh, extrudedMesh, finalSections, edges, invertFaces);
    }
    ...
    public static void ExtrudeMesh(Mesh srcMesh, Mesh extrudedMesh, Matrix4x4[] extrusion, Edge[] edges, bool invertFaces)
    {
        ...
        //if (inputUV.Length == 0)
        //{
        //    inputUV = new Vector2[inputVertices.Length];

        //    for (int i = 0; i < inputVertices.Length; i++)
        //    {
        //        inputUV[i] = new Vector2(inputVertices[i].x, inputVertices[i].z);
        //    }
        //}
